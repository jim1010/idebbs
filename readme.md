...
IdeBBS 是一个简洁的论坛应用，使用 Laravel5.5 编写而成。
...
克隆 `idebbs` 源代码到本地：
    ...
    > git clone https://github.com/jim1010/idebbs.git
...
```
folders:
    - map: ~/my-path/idebbs/ # 你本地的项目目录地址
      to: /home/vagrant/idebbs

sites:
    - map: idebbs.app
      to: /home/vagrant/idebbs/public
...
#### 7. 配置 hosts 文件
    ...
    echo "192.168.10.10   idebbs.app" | sudo tee -a /etc/hosts
...
* 首页地址：http://idebbs.app/
* 管理后台：http://idebbs.app/admin

管理员账号密码如下:

```
username: jim@ideteam.com
password: password
```
...
| 命令行名字 | 说明 | Cron | 代码调用 |
| --- | --- | --- | --- |
| `idebbs:calculate-active-user` |  生成活跃用户 | 一小时运行一次 | 无 |
| `idebbs:sync-user-actived-at` | 从 Redis 中同步最后登录时间到数据库中 | 每天早上 0 点准时 | 无 |
